...
export default function POS() {
  ...
  // Cart panel — shared between desktop sidebar and mobile bottom sheet
  const CartPanel = (
  ...
  return (
    <div className="h-[calc(100vh-56px)] flex flex-col lg:flex-row bg-gray-100 overflow-hidden">
      ...
      {/* Cart panel — fixed bottom sheet on mobile, right sidebar on desktop */}
      <div className={`
      ...
      {/* MOBILE: Floating cart button at bottom */}
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-30 px-4 pb-4">
        <button
          ...
          className={`w-full flex items-center justify-between px-5 py-4 rounded-2xl shadow-xl transition-all duration-300
            ${cart.length > 0 ? 'bg-orange-500 text-white' : 'bg-gray-800 text-gray-400 cursor-default'}`}
        >
